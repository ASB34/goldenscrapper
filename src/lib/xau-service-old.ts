...
export class XauService {
  ...
  private static async fetchFromYahooFinance(currencies: string[]) {
    try {
      ...
      /* Premium API-Ninjas code (for future use):
      ...
      // API-Ninjas returns: { price: 2657.50 } (USD per troy ounce)
      const goldPriceUsdPerOz = data.price;
      if (!goldPriceUsdPerOz) {
        throw new Error('No gold price data received from API-Ninjas');
      }
      ...
      for (const currency of currencies) {
        ...
        const savedRate = await prisma.xauRates.create({
          data: {
            currency,
            rate: parseFloat(goldPricePerGram.toFixed(4)),
            ratePerOz: parseFloat(goldPricePerOz.toFixed(4)),
      ...
      */
      ...
      console.error('❌ Error fetching XAU rates from Yahoo Finance:', error);
      
      // Don't use fallback - throw the error so we know what's wrong
      throw new Error(`Failed to fetch real XAU rates: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  // Fallback rates for development (updated with more realistic values)
  private static getFallbackRates(currencies: string[]) {
    // Updated fallback rates based on approximate current gold prices
    // Gold price around $2650 per troy ounce = ~$85.2 per gram
    const fallbackRates: any = {
      'USD': 85.2,    // USD per gram of gold
      'EUR': 78.5,    // EUR per gram of gold
      'GBP': 67.8,    // GBP per gram of gold
      'TRY': 2895.0   // TRY per gram of gold (34 TRY/USD rate)
    };
    
    console.log('🔄 Using updated fallback XAU rates for development...');
    
    return currencies.map(currency => ({
      id: `fallback-${currency}-${Date.now()}`,
      currency,
      rate: fallbackRates[currency] || 85.0,
      ratePerOz: (fallbackRates[currency] || 85.0) * 31.1035,
